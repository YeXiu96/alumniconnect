...
// ── Supabase Config ─────────────────────────────
// Ganti SUPABASE_URL dan SUPABASE_KEY dengan kredensial Anda
// atau biarkan kosong — modal konfigurasi akan muncul otomatis
...
async function init() {
  ...
  if (!url || !key) {
    // Tampilkan loading sebentar lalu modal config
    setTimeout(() => {
      hidePage('page-loading');
      showConfigModal();
    }, 800);
    return;
  }
  ...
  try {
    ...
    document.getElementById('loading-text').textContent = 'Berhasil terhubung!';
    ...
    document.getElementById('loading-text').textContent = 'Gagal terhubung. Periksa konfigurasi.';
    setTimeout(() => {
      hidePage('page-loading');
      showConfigModal();
    }, 1500);
